...
class VanillaDeepHedgingGym(tf.keras.Model):
    ...
    def _call( self, data : dict, training : bool ) -> dict:
        ...
        pnl  = tf.debugging.check_numerics(pnl, "Numerical error computing pnl in %s. Turn on tf.enable_check_numerics to find the root cause. Note that they are disabled by v in trainer.py" % __file__ )
