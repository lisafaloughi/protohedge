...
from .base import Logger, Config, tf, dh_dtype, Int, fmt_list, DIM_DUMMY
from .layers import DenseLayer, VariableLayer, ClusteredProtoLayer
import numpy as np
import pickle
import os
...
class ProtoAgent(tf.keras.layers.Layer):
    """
    Prototype-Based Action Model for Deep Hedging
    Uses fixed pre-trained prototypes and learns the action associated with each.
    """

    Default_features_per_step = ['price', 'delta', 'time_left']

    def __init__(self, nInst, config, name=None, dtype=dh_dtype):
        super().__init__(name=name, dtype=dtype)
        features = config("features", self.Default_features_per_step, list, "Named features for the agent to use")

        prototype_data_path = config("prototype_path", "./prototypes.pkl", str, "Path to saved prototypes file")
        
        with open(prototype_data_path, "rb") as f:
            data = pickle.load(f)
        self.prototypes = data["prototypes"]  # in standardized space
        self.scaler = data["scaler"]          # StandardScaler object

        self.feature_mean = tf.constant(self.scaler.mean_, dtype=tf.float32)
        self.feature_std  = tf.constant(self.scaler.scale_, dtype=tf.float32)
    ...
    def call(self, all_features, training=False):
        """
        Compute next action using the nearest prototype
        """
        inputs = tf.concat([all_features[k] for k in self.features], axis=-1)  # shape: [?, 3]
        
        # Apply standardization with saved scaler parameters
        # Convert to numpy, scale, then back to tensor
        inputs_standardized = (inputs - self.feature_mean) / self.feature_std   
        actions = self.proto_layer(inputs_standardized)  # shape: [?, nInst]
        return actions, None
    
    def print_sample_prototypes(self, indices=[0, 50, 218]):
        protos = self.proto_layer.prototypes.numpy()
        actions = self.proto_layer.prototype_actions.numpy()
        for i in indices:
            tf.print(f"Prototype {i}: Features = {protos[i]}, Action = {actions[i]}")
    ...
    def description(self):
        return (
            f"ProtoAgent using {self.num_trainable_weights} trainable weights for {self.nInst} instruments.\n"
            f"Features used: {fmt_list(self.features)}"
        )
